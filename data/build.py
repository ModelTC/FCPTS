...
from .folder_sensetime import ImageFolderSensetime
...
def build_loader(config):
    ...
    calib_indices = random.sample(range(len(dataset_train_all)), k=16*64)
    print("calib_indices : ", calib_indices)
    dataset_train = torch.utils.data.Subset(dataset_train_all, calib_indices)
    config.freeze()
...
def build_dataset(is_train, config):
    if config.DATA.DATASET == 'imagenet':
        ...
        if config.DATA.ZIP_MODE:
            ...
            if config.DATA.DATA_FORMAT == 'sensetime':
                dataset = ImageFolderSensetime(data_dir, transform=transform)
            else:
